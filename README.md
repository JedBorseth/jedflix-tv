...
**[Download APK](https://github.com/JedBorseth/jedflix-tv/releases/tag/v0.3.2)** · Leanback, API 24+ · sideload only (not on Play Store)
...
2. Install the APK (`adb install jedflix-tv-0.3.2.apk`, or copy onto the TV).
3. Settings → paste the key, or **Enter from phone** and scan the QR.

**0.2.1 and 0.3.0:** uninstall JedFlix first, then sideload 0.3.2. Those builds were signed with throwaway debug keys, so in-app update cannot replace them. **0.3.1** can update in place.
